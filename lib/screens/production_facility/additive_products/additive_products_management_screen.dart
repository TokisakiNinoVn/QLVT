...
class AdditiveProductsManagementScreen extends StatefulWidget {
  const AdditiveProductsManagementScreen({super.key});

  @override
  State<AdditiveProductsManagementScreen> createState() =>
      _AdditiveProductsManagementScreenState();
}

class _AdditiveProductsManagementScreenState
    extends State<AdditiveProductsManagementScreen> {
  static final AdditiveProductsService _fertilizerMedicineService =
      AdditiveProductsService();
  late List<Map<String, dynamic>> listAdditiveProducts = [];
  late List<Map<String, dynamic>> filteredAdditiveProductss = [];
  bool _isLoading = true;
  String _selectedFilter = 'all'; // Changed from 'role' to 'filter' for clarity
  ...
  void _onSearchChanged() {
    ...
    _debounce = Timer(const Duration(milliseconds: 300), () {
      _filterAdditiveProductss();
    });
  }

  Future<void> _loadAdditiveProductss() async {
    try {
      final response =
          await _fertilizerMedicineService.getAllAdditiveProductsService();
  ...
  void _filterAdditiveProductss() {
    final searchTerm = _searchController.text.toLowerCase();
    setState(() {
      if (_selectedFilter == 'all') {
        filteredAdditiveProductss =
            listAdditiveProducts.where((fertilizer) {
              final name =
  ...
  Map<String, dynamic>? selectedAdditiveProducts;

  void _showAdditiveProductsDetails(Map<String, dynamic> fertilizer) {
    setState(() => selectedAdditiveProducts = fertilizer);
              ...
              body: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    ...
                    ..._buildAdditiveProductsInfoList(fertilizer),
                    Row(
                      ...
                      children: [
                        ElevatedButton.icon(
                          onPressed: () {
                            context.go(
                              "/additive-products-management/additive-products-update/${fertilizer['id']}",
                            );
                          },
            ...
            child: Scaffold(
              ...
              body: Stack(
                children: [
                  Center(
                    child: Hero(
                      tag: "image-${selectedAdditiveProducts?['id']}",
  ...
  List<Widget> _buildAdditiveProductsInfoList(Map<String, dynamic> fertilizer) {
    final entries = {
                ...
                onPressed: () async {
                  ...
                  try {
                    final response = await _fertilizerMedicineService
                        .deleteAdditiveProductsService(fertilizer["id"], {
                          "_method": "DELETE",
                        });
                    if (response["status"] == true || response["code"] == 200) {
                      setState(() {
                        listAdditiveProducts.removeWhere(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: _loadAdditiveProductss,
            tooltip: 'Làm mới',
          ),
          IconButton(
            ...
            onPressed:
                () => context.push(
                  '/additive-products-management/additive-products-create',
                ),
          ...
          Expanded(
            child:
                _isLoading
                    ...
                    : filteredAdditiveProductss.isEmpty
                    ? const Center(child: Text('Không có bản ghi nào'))
                    : ListView.builder(
                      padding: const EdgeInsets.all(8),
                      itemCount: filteredAdditiveProductss.length,
                      itemBuilder: (context, index) {
                        final fertilizer = filteredAdditiveProductss[index];
                        return Card(
                          elevation: 2,
                          ...
                          child: ListTile(
                            onTap:
                                () => _showAdditiveProductsDetails(fertilizer),
