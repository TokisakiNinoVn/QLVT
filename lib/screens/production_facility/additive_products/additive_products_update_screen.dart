    ...
    try {
      ...
      if (rawData != null && mounted) {
        setState(() {
          ...
          if (rawData['hinhanh'] != null && rawData['hinhanh'] != '') {
            String imagePath = rawData['hinhanh'];
            if (imagePath.startsWith('https://')) {
              _networkImageUrl = imagePath;
            } else {
              _networkImageUrl = '${AppConfig.apiUrlImage}/$imagePath';
            }
            _fertilizerMedicineImage =
                null; // Ensure local image is null if using network image
          } else {
            // Trường hợp không có ảnh, có thể xử lý fallback nếu cần
            _networkImageUrl = null;
            // _fertilizerMedicineImage = 'assets/images/default_image.png'; // Ví dụ
          }
        });
      }
    } catch (e) {
