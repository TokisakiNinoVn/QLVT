...
class TransportUpdateScreen extends StatefulWidget {
  final int id;
  const TransportUpdateScreen({super.key, required this.id});
...
class _TransportUpdateScreenState extends State<TransportUpdateScreen> {
  ...
  Future<void> _loadDetailsHarvestPackaging() async {
    ...
    try {
      final response = await _harvestPackagingService.getDetailsHarvestPackagingByIdService(widget.id);
