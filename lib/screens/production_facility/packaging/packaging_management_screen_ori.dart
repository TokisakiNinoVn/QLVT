...
class _PackagingManagementScreenState extends State<PackagingManagementScreen> {
  ...
  // Parse weight and return value in grams for consistent calculations
  Map<String, dynamic> _parseWeight(String? weight) {
    if (weight == null || weight.isEmpty) {
      return {'value': 0, 'unit': 'g'};
    }
    try {
      // Giữ lại số và dấu chấm thập phân
      final cleanedWeight = weight.replaceAll(RegExp(r'[^0-9.]'), '');
      final unit = weight.toLowerCase().contains('kg') ? 'kg' : 'g';
      final value = double.parse(cleanedWeight);
      return {
        'value': unit == 'kg' ? (value * 1000).round() : value.round(),
        'unit': 'g'
      };
    } catch (e) {
      return {'value': 0, 'unit': 'g'};
  ...
  String _formatWeight(int value, String unit) {
    if (unit == 'kg' && value >= 1000) {
      return '${value ~/ 1000}kg';
    }
    return '$value${unit == 'kg' ? 'g' : 'g'}';
  }
  ...
  void _showCreatePackaging(Map<String, dynamic> history) {
    ...
    int selectedProductWeight = 0;
    String selectedProductUnit = 'g';
    int packageCount = 0;
          ...
          builder: (context, setState) => SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: listSanPhamDauRa.map((item) {
                    ...
                    return DropdownMenuItem<String>(
                      ...
                      // child: Text('${item['tensanpham']} - $weight'),
                      child: Text(
                        '${item['tensanpham']} - $weight',
                ...
                const SizedBox(height: 16),
                Text(
                  'Khối lượng thành phẩm còn lại: ${_formatWeight(availableWeight - (packageCount * selectedProductWeight), 'g')}',
                  style: const TextStyle(fontSize: 14, color: Colors.black87),
                ),
            ...
            onPressed: () async {
              ...
              try {
                ...
                Map<String, dynamic> data = {
                  ...
                  'khoiluong_thanhpham': _formatWeight(availableWeight - totalWeight, history['khoiluong_thanhpham']?.toLowerCase().contains('kg') ? 'kg' : 'g'),
                  'trongluong': _formatWeight(totalWeight, selectedProductUnit),
