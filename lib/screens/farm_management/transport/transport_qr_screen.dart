...
class ScanQRTransportScreen extends StatefulWidget {
  const ScanQRTransportScreen({super.key});

  @override
  State<ScanQRTransportScreen> createState() => _ScanQRScreenState();
}

class _ScanQRScreenState extends State<ScanQRTransportScreen> {
  ...
  Future<void> _showConfirmationDialog(String code) async {
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text('Bạn muốn thực hiện hành động nào với mã QR: $code?'),
          actions: <Widget>[
            TextButton(
              child: const Text('Thêm mới'),
              onPressed: () {
                Navigator.of(context).pop();
                // context.push('/transport-management/create-transport/$code');
              },
            ),
    ...
    try {
      ...
      if (value != null && mounted) {
        ...
        if (code.isNotEmpty && mounted) {
          await _showConfirmationDialog(code);
        } else if (mounted) {
          ScaffoldMessenger.of(
            context,
