...
class CareProcessManagementScreen extends StatefulWidget {
  const CareProcessManagementScreen({super.key});

  @override
  State<CareProcessManagementScreen> createState() =>
      _CareProcesstScreenState();
}

class _CareProcesstScreenState extends State<CareProcessManagementScreen> {
  static final CareProcessService _listCareProcessService = CareProcessService();
  static final FertilizerMedicineService _listFertilizerMedicineService = FertilizerMedicineService();
    ...
    _debounce = Timer(const Duration(milliseconds: 300), () {
      setState(() {
        filteredCareProcesses =
            listCareProcesses.where((account) {
  ...
  void _showAccountDetails(Map<String, dynamic> listCareProcess) {
    setState(() => selectedAccount = listCareProcess);
                ...
                body: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildDetailItem(
                        'Thứ tự',
                        listCareProcess["sapxep"].toString(),
                      ),
                      _buildDetailItem(
                        'Mã vùng trồng',
                        listCareProcess["mavungtrong"].toString(),
                      ),
                      _buildDetailItem(
                        'Ngày tạo',
                        _formatDate(listCareProcess["created_at"]),
                      ),
                      _buildDetailItem(
                        'Ngày cập nhật',
                        _formatDate(listCareProcess["updated_at"]),
                      ),
                      _buildDetailItem(
                        'Mã doanh nghiệp',
                        listCareProcess["madoanhnghiep"].toString(),
                      ),
                    ],
                  ),
                ),
  ...
  void _showEditDialog(Map<String, dynamic> listCareProcess) {
    final TextEditingController hoatDongController = TextEditingController(
      text: listCareProcess['hoatdong'],
    );
    final TextEditingController moTaController = TextEditingController(
      text: listCareProcess['mota'],
    );
    int currentOrder = listCareProcess['sapxep'] as int;
              ...
              return Dialog(
                ...
                child: Container(
                  ...
                  child: Scaffold(
                    backgroundColor: Colors.transparent,
                    appBar: AppBar(
                      title: const Text("Chỉnh sửa quy trình chăm sóc"),
                      automaticallyImplyLeading: false,
                      actions: [
                        IconButton(
                          icon: const Icon(Icons.close),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          ElevatedButton(
                            ...
                            child: Text('Chọn từ danh sách có sẵn', style: TextStyle(fontSize: 16, color: ColorConfig.white)),
                          ),
                        ],
                      ),
                    ),
                                ...
                                try {
                                  ...
                                  if (response["status"] == true ||
                                    ...
                                    setState(() {
                                      ...
                                      if (index != -1) {
                                        listCareProcesses[index]["hoatdong"] =
                                            hoatDongController.text;
                                        listCareProcesses[index]["mota"] =
                                            moTaController.text;
                                        listCareProcesses[index]["sapxep"] =
                                            currentOrder;

                                        listCareProcesses.sort(
                                          (a, b) => a['sapxep'].compareTo(
                                            b['sapxep'],
                                          ),
                                        );
                                        filteredCareProcesses = List.from(
                                          listCareProcesses,
                                        );
                                      }
                                    });
                    ...
                    : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: InkWell(
                            borderRadius: BorderRadius.circular(12),
                            onTap: () => _showAccountDetails(listCareProcess),
                            child: Padding(
                              padding: const EdgeInsets.all(16),
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
