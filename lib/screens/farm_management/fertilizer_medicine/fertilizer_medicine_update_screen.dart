...
class FertilizerMedicineUpdateScreen extends StatefulWidget {
  final int id;
  const FertilizerMedicineUpdateScreen({super.key, required this.id});

  @override
  State<FertilizerMedicineUpdateScreen> createState() =>
      _FertilizerMedicineUpdateScreenState();
}

class _FertilizerMedicineUpdateScreenState
    extends State<FertilizerMedicineUpdateScreen> {
  ...
  void initState() {
    super.initState();
    _loadCareProcesses();
    _loadDetailsFertilizerMedicine();
  }
  ...
  Future<void> _loadDetailsFertilizerMedicine() async {
    try {
      setState(() => _isLoading = true);
      final response = await _fertilizerMedicineService
          .getDetailsFertilizerMedicineService(widget.id);
      final rawData = response["data"]?["phanbon_thuoc"];
      if (rawData != null && mounted) {
        setState(() {
          ...
          _selectedCareProcessId = rawData['mahoatdong']?.toString();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        ...
        //     colors: [ColorConfig.primary, Colors.white],
        //   ),
        // ),
        child: Padding(
            ...
            child: ListView(
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  value:
                      listCareProcesses.any(
                            (e) => e['id'].toString() == _selectedCareProcessId,
                          )
                          ? _selectedCareProcessId
                          : null,
                  onChanged: (value) {
                  ...
                  items:
                      listCareProcesses.map((careProcess) {
                            ...
                            if (_formKey.currentState!.validate()) {
                              ...
                              try {
                                final result = await _fertilizerMedicineService
                                    .updateFertilizerMedicineService(
                                      widget.id,
                                      data,
