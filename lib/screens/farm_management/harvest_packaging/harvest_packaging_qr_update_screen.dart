...
class ScanQRUpdateHarvestPackagingScreen extends StatefulWidget {
  const ScanQRUpdateHarvestPackagingScreen({super.key});

  @override
  State<ScanQRUpdateHarvestPackagingScreen> createState() => _ScanQRScreenState();
}

class _ScanQRScreenState extends State<ScanQRUpdateHarvestPackagingScreen> {
    ...
    try {
      ...
      if (value != null && mounted) {
        ...
        if (code.isNotEmpty && mounted) {
          context.go('/harvest-packaging-management/update-harvest-packaging/$code');
        } else if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            ...
            child: Column(
              children: [
                if (_isProcessing)
                  ...
                  const Text(
                    'Đưa mã QR lô hàng muốn cập nhật vào vùng quét',
